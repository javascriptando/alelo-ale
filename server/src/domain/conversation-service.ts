...
async function tryCaptureNps(
  ...
  await gateway.sendText(phone, thanks).catch(() => {})
...
export function makeInboundHandler(gateway: WhatsAppGateway) {
  return async (inbound: InboundMessage) => {
    ...
    if (await tryCaptureNps(client.id, inbound.text, gateway, inbound.phone)) {
      return
    }
    ...
    if (reply.text) {
      ...
      const stored = await storeMessage(convo.id, client.id, 'bot', reply.text, {
      ...
      if (stored) await sendAndMark(gateway, stored.id, inbound.phone, reply.text)
    }
  }
}
